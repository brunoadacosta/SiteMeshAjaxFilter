 ...
 * 
 * <p>
 * The page attribute should point to the panel resource which should expose an
 * entire page (e.g. another JSP file producing HTML). This attribute can be
 * relative to the page it is being called from or an absolute path from the
 * context-root.
 * </p>
 * 
 * <p>
 ...
 * {@link com.opensymphony.module.sitemesh.Page} object and has the
 * {@link com.opensymphony.module.sitemesh.Decorator} applied.
 * </p>
 * 
 * <p>
 * The (optional) decorator attribute is the name of the
 * {@link com.opensymphony.module.sitemesh.Decorator} to apply to the included
 * page. Note that the implementation of
 * {@link com.opensymphony.module.sitemesh.DecoratorMapper} can overide this.
 * </p>
 * 
...
public class ApplyDecoratorTag extends BodyTagSupport implements RequestConstants {
	private static final long serialVersionUID = 5652635797732122457L;
	...
	/**
	 * Tag attribute: URI of page to include. Can be relative to page being
	 * called from, or absolute path from context-root of web-app.
	 */
	...
	/**
	 * Add a parameter to the page. This has a package level access modifier so
	 * ParamTag can also call it.
	 */
	...
	/**
	 * Tag attribute: If set, this value will override the 'title' property of
	 * the page. This is a convenience utility and is identical to specifing a
	 * 'page:param name=title' tag.
	 */
	...
	/**
	 * Tag attribute: If set, this value will override the 'id' property of the
	 * page. This is a convenience utility and is identical to specifing a
	 * 'page:param name=id' tag.
	 */
	...
	/**
	 * Tag attribute: Name of Decorator to apply to Page. This is passed to
	 * DecoratorMapper to retrieve appropriate Decorator. DecoratorMapper may
	 * override if needed.
	 * 
	...
	public int doEndTag() throws JspException {
		if (!isAjax((HttpServletRequest) pageContext.getRequest())) {
			try {
				...
				if (page == null) {
					...
					if (fullPath.length() > 0 && fullPath.charAt(0) != '/') {
						...
						// check if it did not return null (could occur when the
						// servlet container
						// does not use a servlet to serve the requested
						// resouce)
						if (thisPath == null) {
					...
					PageRequestWrapper pageRequest = new PageRequestWrapper(
					...
					// if rd == null, then the panel was not found, but this
					// isn't
					// correct, so we need to spit out
					// something appropriate. What this is, well...I don't know
					// yet.
					if (rd == null) {
						throw new ApplyDecoratorException("The specified resource in applyDecorator tag (" + fullPath
								+ ") was not found.");
					}
				...
				// If pageObj == null, then the panel source had some weird
				// error in
				// it. Stop writing bugs like this. They're ugly and they make
				// you
				// smell funny.
				if (pageObj == null) {
				...
				params.clear(); // params need to be cleared between invocations
								// -
								// SIM-191
			} catch (IOException e) {
				throw new JspException(e);
